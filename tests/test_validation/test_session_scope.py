...
from opentide.validation.session import (
    _id_duplicate_in_scope,
    _IdScanRow,
    _merge_id_duplicates,
    _scope_has_narrow_filter,
    _scope_no_match_issue,
    run_validation,
)


def test_scope_has_narrow_filter() -> None:
    assert not _scope_has_narrow_filter(ValidationScope.full())
    assert _scope_has_narrow_filter(ValidationScope.narrow(uuids=frozenset({"x"})))


def test_scope_no_match_issue_fields() -> None:
    scope = ValidationScope.narrow(uuids=frozenset({"missing"}))
    issue = _scope_no_match_issue(scope)
...
def test_id_duplicate_in_scope_file_target() -> None:
    ...
    row = _IdScanRow(Path("/repo/a.yaml"), "rule", "u1", "A")
    original = _IdScanRow(Path("/repo/b.yaml"), "rule", "u1", "B")
    assert _id_duplicate_in_scope(scope, row, original)


def test_id_duplicate_unrelated_files_skipped() -> None:
    scope = ValidationScope.narrow(files=frozenset({"other.yaml"}))
    row = _IdScanRow(Path("a.yaml"), "rule", "u1", "A")
    original = _IdScanRow(Path("b.yaml"), "rule", "u1", "B")
    assert not _id_duplicate_in_scope(scope, row, original)


def test_merge_id_duplicates_respects_scope() -> None:
    scope = ValidationScope.narrow(files=frozenset({"new.yaml"}))
    scans = [
        _IdScanRow(Path("existing.yaml"), "rule", "dup", "Existing"),
        _IdScanRow(Path("new.yaml"), "rule", "dup", "New"),
    ]
    issues = _merge_id_duplicates(scans, scope)
    assert len(issues) == 1
