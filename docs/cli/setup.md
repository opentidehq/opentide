...
`yaml.schemas` maps each object folder to its concrete schema (`threat.1.0.schema.json` → `objects/threats/*.yaml`, and the same for objectives and rules). It does not map the `opentide.schema.json` router against `objects/**/*.yaml` — the Red Hat YAML extension reports "Matches multiple schemas" for that glob.
