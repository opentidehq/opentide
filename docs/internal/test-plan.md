...
- **Published commands** (`tests/test_docs/test_docs_commands_golden.py`): extracts every `opentide …` invocation from the shell fences in `docs/` (`tests/docs_commands.py`) and resolves it against the live Typer tree, executes the read-only subset against a scaffolded repo, and compares the documented `text`/`json` output fences to real output. The YAML-fence tutorial test covers the objects; this covers the commands, which is the half that let #247 ship.
