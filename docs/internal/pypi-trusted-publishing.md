...
Cut a patch on `development` after the changelog and release notes land (example: **0.1.4**):
...
gh release create v0.1.4 \
  ...
  --target development \
  --title "0.1.4" \
  --notes-file .github/release-notes/v0.1.4.md
```

The tag must sit on the merge commit that contains `CHANGELOG.md`, `docs/usage/releases.md`, and `.github/release-notes/v0.1.4.md`. Do not tag a feature branch.
