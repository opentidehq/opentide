...
from opentide.models.legacy import TideDefinitionsModels, TideModels, SystemConfig, DeploymentStrategy, StatusStrategy, TenantDeployment, TenantDeploymentModel
...
import structlog
logger = structlog.get_logger('opentide.deployment.git_repo')
...
class GitRepository:
    ...
    def _latest_commit_information(self) -> LatestCommit:
        if CIEnvironment().environment is CIEnvironment.CIPlatforms.LocalDebug:
            return self.LatestCommit(message='Sample Commit Message', author='Sample Commit Author', sha='Sample Commit SHA')
        commit = self.repository.head.commit
        return self.LatestCommit(message=str(commit.message.strip()), author=str(commit.author.name), sha=str(commit.hexsha))
