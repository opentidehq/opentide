    ...
    def fetch_config_envvar(config_secrets: dict[str, str]) -> dict[str, Any]:
        ...
        if DebugHelpers.is_debug():
            try:
                ...
                log('FAILURE', 'Could not find local python file at `opentide.core.local_secrets` to set secret environment variables', 'Parts of this module may not work properly', 'Refer to the relevant TOML conguration file to find which variables may be necessary')
        for sec in config_secrets.copy():
            if not config_secrets[sec]:
                log('SKIP', 'Did not found an entry for', sec, 'If there are deployment issue, review if it is relevant to configure')
                continue
            if type(config_secrets[sec]) == str:
                if config_secrets[sec].startswith('$'):
                    if config_secrets[sec].removeprefix('$') in os.environ:
                        ...
                        log('SKIP', 'Could not find expected environment variable', config_secrets[sec], 'Debug Mode identified, continuing - remember that this may break some deployments')
                    else:
                        log('FATAL', 'Could not find expected environment variable', config_secrets[sec], 'Review configuration file and execution environment')
                        missing_envvar_error = True
        if missing_envvar_error:
            log('FATAL', 'Some environment variable specified in configuration files were not foundReview the previous errors to find which ones were missing', 'Check your CI settings to ensure these environment variables are properly injected', "This may not be a critical issue, for example if you didn't enable a particular system")
