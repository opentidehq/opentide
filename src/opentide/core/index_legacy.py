    ...
    def reconcile_staging(index):
        ...
        for mdr in STG_INDEX:
            if mdr not in RECONCILED_INDEX['objects']['rule']:
                ...
                main_mdr_metadata = RECONCILED_INDEX['objects']['rule'][mdr].get('meta') or RECONCILED_INDEX['objects']['rule'][mdr]['metadata']
                ...
                if stg_version > main_version:
                    log('INFO', f' Replacing MDR {mdr_name} from prod index with staging data, as version is higher (main : v{main_version} staging : v{stg_version})')
