...
import structlog
logger = structlog.get_logger('opentide.indexing.revisions')
...
class RevisionIndexer:
    ...
    def _new_revision(self, object_author: str='') -> RevisionTracker:
        ...
        return RevisionTracker(date=datetime.today().strftime('%Y-%m-%d'), message=commit_details.message, author=commit_details.author, commit=commit_details.sha)
    ...
    def _create_entry(self, object: str, object_type: str):
        ...
        if (existing_entry := self.REVISIONS_INDEX.get(object)):
            ...
            return RevisionIndexEntry(name=name, object=object_type_name, description=description, revisions={object_version: self._new_revision()})
