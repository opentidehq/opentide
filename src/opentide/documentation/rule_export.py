...
def document_detection_rule(rule: DetectionRule) -> str:
    ...
    lines = [f'# {rule.name}', '', rule.description, '', '## Metadata', '', f'- **UUID**: `{rule.metadata.uuid}`', f'- **Schema**: `{rule.metadata.schema_id}`', f'- **Status**: {rule.status}', f'- **Severity**: {rule.severity}', '']
    if rule.techniques:
    ...
    if rule.configurations:
        ...
        lines.append('')
        for platform_name in ('sentinel', 'defender_for_endpoint', 'splunk', 'sentinel_one', 'crowdstrike', 'harfanglab', 'carbon_black_cloud'):
