...
logger = structlog.get_logger('opentide.documentation.wiki_navigation')
start_time = time.time()
from opentide.generation.framework import techniques_resolver, relations_list, get_type, get_vocab_entry, keep_active_mdr
from opentide.documentation.core import model_value_doc, get_icon, get_field_title, make_json_table, backlink_resolver, rich_attack_links, DOCUMENTATION_TARGET
...
CHARS_CLIP = 150
NAV_INDEX_FIELDS = {'tvm': ['uuid', 'name', 'criticality', 'tlp', 'description', 'modified', 'implementations', 'criticality', 'surface', 'att&ck', 'actors', 'cve', 'impact', 'leverage'], 'dom': ['uuid', 'name', 'priority', 'tlp', 'att&ck', 'objective', 'modified', 'threats', 'implementations'], 'mdr': ['uuid', 'name', 'description', 'modified', 'statuses', 'att&ck', 'detection_model']}
...
def build_search(model_type, mdr_status: Optional[Literal['ACTIVE', 'DEPRECATED']]=None):
    ...
    custom_cols = [system_column, implementation_column, schema_version, mdr_attack_technique, mdr_statuses]
    for entry in index_data:
        ...
        for value in NAV_INDEX_FIELDS[model_type]:
            if value == 'name':
                ...
                objective_section: dict = model_value_doc(entry, 'objective')
                ...
                if value == 'statuses':
                    ...
                    for system in configurations:
                        sys_status = configurations[system]['status']
                        if mdr_status == 'ACTIVE' and sys_status not in DEPRECATED_STATUSES:
                            ...
                            statuses[system] = sys_status
                    statuses = ', '.join([f'{k.capitalize()}:{v}' for k, v in statuses.items()])
                    row[mdr_statuses] = statuses
                elif value == 'att&ck':
                    model_value = techniques_resolver(str(model_value_doc(entry, 'uuid'))) or ''
                    if model_value:
                    ...
                    model_value = model_value_doc(entry, value, with_icon=True, max_chars=CHARS_CLIP)
                ...
                if type(model_value) == list:
                    ...
                    for v in model_value_iter:
                        if model_value_iter[v] == None or model_value_iter[v] == [None]:
                            model_value.pop(v)
                    if model_value != {}:
                        flat_values = [k.capitalize() + ' : ' + ', '.join(model_value[k]) for k in model_value]
    ...
    rename_mapping = {c: f"{get_field_title(c, METASCHEMAS_INDEX[model_type]['properties'])}" for c in [x for x in df.columns if x not in custom_cols]}
...
def construct_navigation_index(model):
    ...
    if model == 'mdr':
        ...
        active_mdr_summary = CENTER_TEXT.format(icon=icon, count=active_mdr_count, model_title=active_mdr_title)
        active_mdr_details = build_search(model, mdr_status='ACTIVE')
        deprecated_mdr_title = 'Deprecated ' + model_title
        deprecated_mdr_summary = CENTER_TEXT.format(icon=icon, count=deprecated_mdr_count, model_title=deprecated_mdr_title)
        ...
        summary = CENTER_TEXT.format(icon=icon, count=count, model_title=model_title)
        details = build_search(model)
        nav_index = summary + '\n\n' + details
    return nav_index

def run():
    emit_section('Wiki Navigation Index')
    logger.info('assembles_tables_exposing_coretide_data_to_make_the_dataset_easi')
    if not COVER_PAGES_ENABLED:
        logger.info('disabled_in_configuration', detail='Not generating cover pages as set to false or missing key', advice='You can enable this feature by setting model_cover_pages to True in documentation.toml')
        return
    if not os.path.exists(MODELS_DOCS_PATH):
        logger.info('create_wiki_and_documentation_folder')
        MODELS_DOCS_PATH.mkdir(parents=True)
    for model in MODELS:
        logger.info('generating_navigation_index_for_model_type', arg0=model)
    ...
    logger.info('step_completed', detail=f'Generated navigation index in {time_to_execute} seconds')
