...
from opentide.documentation.core import get_field_title, get_icon, rich_attack_links, get_vocab_description, GitlabMarkdown, model_value_doc, FOLD
from opentide.core.files import safe_file_name
from opentide.documentation.templates.mdr import TEMPLATEv3
from opentide.documentation.components import tlp_doc, metadata_doc, relations_table, reference_doc, status_enriched
...
from opentide.documentation.core import TARGET_WITH_DASH_PATHS, DOCUMENTATION_TARGET, UUID_PERMALINKS
...
def documentation(mdr):
    ...
    if isinstance(mdr, DetectionRule):
        ...
        if mdr.get('configurations'):
            mdr['configurations'] = {key: value for key, value in mdr['configurations'].items() if value is not None}
    doc = str()
    ...
    for s in mdr_configs:
        ...
        system_data = pd.json_normalize(mdr_configs[s], sep='|').to_dict(orient='records')[0]
        query = system_data.pop('query').strip()
        expander = f'Expand to view {system_name} query'
        queries += QUERY_FOLD.format(expander, query)
        for key in system_data.copy():
            if key == 'status':
                continue
            buffer = dict()
            cleaned_key = str(key).split('|')[-1]
            key_name = get_field_title(cleaned_key, SYSTEMS_SUBSCHEMAS[s])
            param_description = get_value_metaschema(cleaned_key, metaschema=SYSTEMS_SUBSCHEMAS[s], retrieve='description') or ''
            param_name = get_value_metaschema(cleaned_key, metaschema=SYSTEMS_SUBSCHEMAS[s], retrieve='tide.mdr.parameter')
            param_vocab = get_value_metaschema(cleaned_key, metaschema=SYSTEMS_SUBSCHEMAS[s], retrieve='tide.vocab')
            ...
            if param_vocab:
                ...
                if cleaned_key == 'status':
                    ...
                    enriched_value = model_value_doc(uuid_data, cleaned_key, with_icon=True)
                data = f'**{enriched_value}** : {get_vocab_description(vocabulary, data)}'
            else:
                if type(data) is str:
                    data = f'`{data}`'
            ...
            buffer['Description'] = str(param_description).replace('$', '\\$').replace('\n', ' ')
            buffer['Config'] = str(data).replace('$', '\\$').replace('\n', ' ')
        ...
        if s not in enabled_systems():
            ...
            banner = 'This system is not enabled in your System Configurations, this documentation is only informational'
            if DOCUMENTATION_TARGET is CIEnvironment.CIPlatforms.GitlabCI:
    ...
    doc = TEMPLATEv3.format(frontmatter=frontmatter, name=name, tlp=tlp, techniques=techniques, metadata=metadata, description=description, relations=relations, response=response, configurations=configurations, queries=queries, references=references)
    return doc
