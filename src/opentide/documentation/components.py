...
import structlog
logger = structlog.get_logger('opentide.documentation.components')
...
def _dom_downstream_rules_table(dom_id: str) -> str:
    ...
    """
    signal_ids = sorted([child for child in childs(dom_id) if get_type(child) == 'signal'], key=object_name)
    direct_mdr_ids = sorted([child for child in childs(dom_id) if get_type(child) == 'mdr'], key=object_name)
    rows = []
    for signal_id in signal_ids:
        mdr_ids = sorted(keep_active_mdr(childs(signal_id)), key=object_name)
        rows.append({'signal': backlink_resolver(signal_id, current_page='dom'), 'mdr': '<br>'.join((backlink_resolver(mdr_id, current_page='dom') for mdr_id in mdr_ids)) if mdr_ids else None})
    active_direct_mdrs = keep_active_mdr(direct_mdr_ids)
    if active_direct_mdrs:
        rows.append({'signal': DOM_DIRECT_MDR_LABEL, 'mdr': '<br>'.join((backlink_resolver(mdr_id, current_page='dom') for mdr_id in active_direct_mdrs))})
    if not rows:
        return ''
...
def status_enriched(status_name: str) -> str:
    ...
    for status in statuses:
        if status_name == status.name:
            ...
            return description
    logger.critical('could_not_look_up_requested_status_in_existing_statuses', detail=f'Requested status : {status_name}', advice=f'Available statuses in deployment.toml : {statuses}')
    raise Exception
...
def actors_doc(actors: list[dict]) -> str:
    ...
    for stage in actor_vocab_stages:
        sources[stage.get('id')] = stage.get('icon') + ' ' + stage.get('name')
    for actor in actors:
        details = {}
        ...
        if not isinstance(actor_data, dict):
            logger.warning('event', detail=f'Could not retrieve actor data for identifier [{clean_identifier}] — skipping actor', arg0=actor_name)
            continue
        details['Actor'] = actor_data.get('name')
        details['Description'] = str(actor_data.get('description')).replace('\n', '')
        details['Aliases'] = ', '.join(actor_data.get('alias', []))
...
def metadata_doc(metadata: dict, model_type: str) -> str:
    ...
    for key, value in metadata.items():
        ...
        if meta_title:
            ...
            logger.warning('event', detail=f'Missing title in metaschema : {metaschema} for key : {key}')
...
def reference_doc(references: dict) -> str:
    ...
    for scope in references:
        if not references[scope]:
            continue
        scope_title = get_field_title(scope, DEFINITIONS_INDEX['references']['properties'])
        references_list = [f'[_{k}_] {v}' for k, v in references[scope].items()]
        reference_labels.extend([f'[{k}]: {v}' for k, v in references[scope].items()])
...
def relations_table(id: str, direction: Literal['upstream', 'downstream']='downstream', raw_data=False):
    ...
    def unfold_trunk(trunk):
            ...
            if trunk:
                ...
                for branch in sorted(trunk):
                    ...
                    if branch_data.get(branch_type):
                        branch_data[branch_type] = branch_data[branch_type] + '<br>' + backlink_resolver(branch, current_page=current_page)
                    else:
        ...
        if type(trunk) is list:
            if trunk:
                branch_data = {}
                branch_data[get_type(trunk[0])] = '<br>'.join([backlink_resolver(b, current_page=current_page) for b in trunk if b != 'Unknown'])
                return branch_data
    data = []
    if type(tree) is list:
        tree = {id: tree}
    for trunk in sorted(tree):
        trunk_data = unfold_trunk({trunk: tree[trunk]}) or {}
        if direction == 'downstream':
            if model_type == 'tvm':
                trunk_data['dom'] = None if 'dom' not in trunk_data else trunk_data['dom']
            if model_type in ['tvm', 'dom']:
                trunk_data['mdr'] = None if 'mdr' not in trunk_data else trunk_data['mdr']
            if model_type == 'dom':
                trunk_data['signal'] = None if 'signal' not in trunk_data else trunk_data['signal']
        elif direction == 'upstream':
            if model_type in ['mdr', 'dom']:
                trunk_data['tvm'] = None if 'tvm' not in trunk_data else trunk_data['tvm']
    ...
    for field in model_data:
        if field not in SKIP_KEYS and field not in SURFACE_KEYS:
            ...
            if type(value) is str or (type(value) is list and len(value) == 1):
                ...
                value_doc = [f'{make_vocab_link(field, key)} : {get_vocab_description(field, key)}' for key in value]
                value_doc = ' - ' + '\n - '.join(value_doc)
            ...
            if field_title and field_description and value_doc:
                ...
                metadata_doc += f'\n\n {value_doc}'
                metadata_doc += '\n\n---\n\n'
    tag_list = [x.replace(' &', '').replace('(', '').replace(')', '').replace(' ', '_') for x in tag_list]
...
def tlp_doc(tlp_data: str, description: bool=True) -> str:
    ...
    tlp_doc_markdown = f'{tlp_icon} **TLP:{tlp_data.upper()}** {tlp_rating_icon}{tlp_description}'
    ...
    if GET_CVE_DETAILS:
        ...
        for vuln in cve_list:
            try:
                ...
                logger.warning('retrieving_cve_details_failed', detail=str(error))
                broken_cve.append(vuln)
        if not broken_cve:
            cve_data = pd.DataFrame(cve_details_list).to_markdown(index=False)
    if not GET_CVE_DETAILS or broken_cve:
        cve_doc_list = [f'[{vuln}]({CVE_DB_LINK}{vuln})' for vuln in cve_list if vuln not in broken_cve]
        if broken_cve:
    ...
    if visibility and visibility.assets:
        ...
        for asset in visibility.assets:
            ...
            if matches:
                ...
                asset_rows.append({'Asset': asset.name, 'Criticality': asset.criticality, 'Matching Surface': ', '.join(matches), 'Visibility': visibility_status})
        if asset_rows:
                ...
                if visibility.logsources:
                    for ls in visibility.logsources:
                        if ls.assets:
                            ...
                            if covered:
                                logsource_data.append({'Log Source': ls.name, 'System': ls.system, 'Covered Assets': ', '.join(covered)})
                if logsource_data:
                ...
                if visibility.detectors:
                    for det in visibility.detectors:
                        if det.assets:
                            ...
                            if covered:
                                detector_data.append({'Detector': det.name, 'Covered Assets': ', '.join(covered)})
                if detector_data:
