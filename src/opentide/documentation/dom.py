...
from opentide.generation.framework import get_vocab_stage_details
from opentide.documentation.core import get_vocab_description
from opentide.core.files import safe_file_name
from opentide.documentation.templates.dom import DETECTION_OBJECTIVE_TEMPLATE, SIGNAL_TEMPLATE
from opentide.documentation.components import tlp_doc, metadata_doc, attack_techniques, frontmatter_doc, relations_table, reference_doc
...
from opentide.documentation.core import TARGET_WITH_DASH_PATHS, DOCUMENTATION_TARGET, UUID_PERMALINKS, get_icon
import structlog
logger = structlog.get_logger('opentide.documentation.dom')
...
class DetectionObjectivesWiki:
    ...
    def __init__(self):
        ...
        if DOCUMENTATION_TARGET in TARGET_WITH_DASH_PATHS:
            ...
            logger.info('going_to_create_file_paths_with_dashes_instead_of_space_for_the', detail=str(DOCUMENTATION_TARGET))
    ...
    def _create_wiki_page(self, objective: Objects.DetectionObjective) -> str:
        ...
        return DETECTION_OBJECTIVE_TEMPLATE.format(frontmatter=frontmatter, name=f"# {get_icon('dom')} " + objective.name if not UUID_PERMALINKS else '', priority=objective.objective.priority, tlp=tlp, techniques=techniques, metadata=metadata, objective_type=objective.objective.type, objective_type_description=objective_type_description, description=objective.objective.description.replace('\n', '\n> '), strategy=objective.objective.composition.strategy.replace('\n', '\n> '), strategy_description=strategy_description, composition_description=objective.objective.composition.description, relation_graph=relation_graph, related_threat_vectors=related_threat_vectors, related_detection_rules=related_detection_rules, signals_list=signals_list, references=references)
    ...
    def _create_signal_content(self, signal: Objects.DetectionObjective.Objective.Signal) -> str:
        ...
        return SIGNAL_TEMPLATE.format(name=signal.name, uuid=signal.uuid, description=signal.description, data_availability=signal.data.availability, data_requirements=signal.data.requirements, logsource_table=logsource_table, entities_table=entities_table, detectors_table=detectors_table, examples_table=examples_table)
