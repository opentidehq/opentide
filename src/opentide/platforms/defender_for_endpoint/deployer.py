...
import structlog
logger = structlog.get_logger('opentide.platforms.defender_for_endpoint.deployer')
...
class DefenderForEndpointDeploy(RuleDeployer):
    ...
    def deploy_mdr(self, data: DetectionRule, service: DefenderForEndpointService, tenant: str):
        ...
        if mdr_config.rule_id:
            ...
            logger.info('event', detail=f'Retrieved ID for tenant {tenant} in MDR', context=str(rule_id), advice='Will perform an update')
        else:
            logger.info('event', detail=f'Could not retrieve ID for tenant {tenant} in MDR', context_1='Will create a new rule, and write back the ID to the file')
            rule_id = None
        if check_status(mdr_config.status) is StatusStrategy.DELETION:
            if not rule_id:
                ...
                logger.info('step_in_progress', detail=f'Proceeding with deletion of rule against tenant {tenant}', context=str(rule_id))
                ...
                with open(file_path, 'w', encoding='utf-8') as mdr_file:
                    logger.info('step_completed', detail=f'Removed ID in MDR File for tenant {tenant}')
            ...
            updated_content = list()
            for line in content:
                logger.debug('event', detail=line)
                if line.strip() == 'defender_for_endpoint:':
                    updated_content.append(line)
                    updated_content.append(f'    rule_id::{tenant}: {rule_id}\n')
                    logger.debug('appending_line', detail=str(rule_id))
                else:
                    updated_content.append(line)
            with open(file_path, 'w', encoding='utf-8') as mdr_file:
                logger.info('step_completed', detail=f'Updated MDR File with new ID for tenant {tenant}', context=str(rule_id))
                mdr_file.writelines(updated_content)
    ...
    def deploy(self, mdr_deployment: Sequence[DetectionRule], deployment_plan: DeploymentStrategy):
        ...
        for tenant_deployment in deployment.rule_deployment:
            ...
            logger.info('planned_deployment', detail=tenant_deployment.tenant.name, advice=str(tenant_deployment.rules))
