...
class CarbonBlackCloudQueryValidator(CarbonBlackCloudConnection, QueryValidator):
    ...
    def check_query(self, mdr: dict, service: CBCloudAPI):
        ...
        try:
            ...
            if result:
                ...
                log('FATAL', f"The CBC query is invalid for : {mdr['name']} ({mdr_uuid})", 'Ensure a value is included and slashes, colons, and spaces are manually escaped')
    ...
    def validate(self, deployment: list[str], deployment_plan=None):
        ...
        log('SUCCESS', 'Successfully connected to Carbon Black Cloud on tenant', self.VALIDATION_ORGANIZATION)
        for mdr in deployment:
            ...
            if self.DEPLOYER_IDENTIFIER in mdr_data['configurations'].keys():
                log('ONGOING', 'Validating CBC Lucene Query', f"{mdr_data['name']} ({mdr_uuid}")
