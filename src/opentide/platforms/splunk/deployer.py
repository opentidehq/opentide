...
from opentide.generation.framework import get_value_metaschema, techniques_resolver
...
import structlog
logger = structlog.get_logger('opentide.platforms.splunk.deployer')
...
class SplunkDeploy(SplunkConnection, RuleDeployer):
    ...
    def config_mdr(self, mdr):
        ...
        for key in mdr_splunk:
            param_name = get_value_metaschema(key, self.SPLUNK_SUBSCHEMA, 'tide.mdr.parameter')
            data = mdr_splunk[key]
            if key == 'lookback':
            ...
            if key == 'frequency':
                if 'cron' not in mdr_splunk:
                    ...
                    param_name = get_value_metaschema('cron', self.SPLUNK_SUBSCHEMA, 'tide.mdr.parameter')
            if type(data) == list:
                data = ', '.join(data)
            if type(data) == str:
                data = data.strip()
        ...
        if risk:
            risk_config = []
            risk_param = get_value_metaschema('risk', self.SPLUNK_SUBSCHEMA, 'tide.mdr.parameter')
            risk_objects_config = []
            ...
            if (ro := risk.get('risk_objects')):
                for risk_object in ro:
                    ...
                    for key in risk_object:
                        risk_object_paramed[get_value_metaschema(key, self.SPLUNK_SUBSCHEMA, 'tide.mdr.parameter', scope='risk_objects')] = risk_object[key]
            ...
            if (to := risk.get('threat_objects')):
                for threat_object in to:
                    ...
                    for key in threat_object:
                        threat_object_paramed[get_value_metaschema(key, self.SPLUNK_SUBSCHEMA, 'tide.mdr.parameter', scope='threat_objects')] = threat_object[key]
            ...
            if risk_message:
                config[get_value_metaschema('message', self.SPLUNK_SUBSCHEMA, 'tide.mdr.parameter', scope='risk')] = risk_message
        if advanced_config:
    ...
    def deploy_mdr(self, mdr, service):
        ...
        if status_modifiers:
            if 'allowed_actions' in status_modifiers:
                ...
                if allowed_actions_config in [False, None]:
                    logger.info('this_mdr_will_not_have_any_action_enabled_in_splunk_as_actions_e', arg0=status)
                    status_allowed_actions = []
                else:
                    logger.info('the_enabled_actions_for_this_mdr_will_be_constrained_by_the_stat', arg0=allowed_actions_config)
                    status_allowed_actions = allowed_actions_config
            if status_modifiers:
                logger.info('status_modifiers_applied', modifiers=status, detail=str(status_modifiers))
        ...
        logger.info('the_following_configuration_was_compiled')
        ...
        try:
            ...
            logger.info('found_existing_saved_search', arg0=name)
        except:
            if check_status(status) is StatusStrategy.DELETION:
                logger.info('saved_search_absent', arg0=name)
                return None
            else:
                logger.info('will_create_a_new_saved_search', arg0=name)
                selected_search = service.saved_searches.create(name, search=query)
        if check_status(status) is StatusStrategy.DELETION:
            service.saved_searches.delete(name)
            logger.warning('splunk_alert_deleted', arg0=name)
            return None
        if self.DEBUG_STEP:
            for k, v in deploy_config.items():
                logger.info('config_value_updating', key=k, value=v)
        ...
        logger.info('deployed_on_splunk', arg0=name)
        return True
