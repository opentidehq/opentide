...
import structlog
logger = structlog.get_logger('opentide.export.attack_navigator_layer')
    ...
    def generate_technique_layer(self) -> list[TechniqueLayer]:
        ...
        for technique in tvm_techniques:
            ...
            if technique not in mdr_techniques:
                logger.info('event', detail=f'{technique} only at TVM level, found in : ', context=str(mapping_detail.objects_names))
                ...
                logger.info('event', detail=f'{technique} Fully Mapped, found in : ', context=str(updated_mapping.objects_names))
        ...
        for technique in mdr_techniques:
            if technique in tvm_techniques:
                continue
            mdr_mapping_detail = mdr_techniques[technique]
            logger.info('event', detail=f'{technique} only at MDR level, found in : ', context=str(mdr_mapping_detail.objects_names))
