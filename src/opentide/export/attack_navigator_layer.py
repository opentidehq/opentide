...
class NavigatorLayer:
    ...
    legendItems: list[LegendEntry] | None = None
...
class AttackNavigatorLayer:
    ...
    def map_objects_and_techniques(
        self, model_type: Literal["threat", "rule"]
    ) -> dict[str, TechniqueIndexEntry]:
        match model_type:
            case "threat":
        ...
        for object in index:
            ...
            for technique in techniques:
                if technique in technique_mapping:
                    technique_mapping[technique].objects_names.append(
                        f"[{model_type.upper()}] " + name
                    )
                    technique_mapping[technique].objects_uuids.append(uuid)
                else:
                    technique_mapping[technique] = TechniqueIndexEntry(
                        objects_names=[f"[{model_type.upper()}] " + name], objects_uuids=[uuid]
                    )
    ...
    def generate_technique_layer(self) -> list[TechniqueLayer]:
        ...
        for technique in tvm_techniques:
            ...
            if technique not in mdr_techniques:
                logger.info(
                    "technique_tvm_only",
                    technique=technique,
                    objects=str(mapping_detail.objects_names),
                )
                technique_layer.append(
                    TechniqueLayer(
                ...
                logger.info(
                    "technique_fully_mapped",
                    technique=technique,
                    objects=str(updated_mapping.objects_names),
                )
                technique_layer.append(
                    TechniqueLayer(
                        techniqueID=technique,
        ...
        for technique in mdr_techniques:
            ...
            logger.info(
                "technique_mdr_only",
                technique=technique,
                objects=str(mdr_mapping_detail.objects_names),
            )
            technique_layer.append(
                TechniqueLayer(
                    techniqueID=technique,
    ...
    def assemble_full_layer(self, technique_layer: list[TechniqueLayer]) -> NavigatorLayer:
        ...
        return NavigatorLayer(
            versions={"layer": "4.5"}, techniques=technique_layer, legendItems=legend
        )
