...
D3FEND_COUNTERMEASURES_PATH = OpenTide.Configurations.Resources.d3fend['countermeasures']
...
D3FEND_COUNTERMEASURES = pd.read_csv(RESOURCES / D3FEND_COUNTERMEASURES_PATH).fillna('').to_dict(orient='records')
...
for c in D3FEND_COUNTERMEASURES:
    ...
    if entry['description'] == '':
        entry['description'] = 'No description exposed by D3FEND in their .csv currently - visit the link for more description about the countermeasure'
...
with open(VOCABS_PATH / OUT_NAME, 'w+', encoding='utf-8') as out:
    ...
    yaml.dump(vocab_file, out, sort_keys=False, Dumper=IndentFullDumper, allow_unicode=True)
