...
def promotion_steps(options: CiRenderOptions) -> list[str]:
    if not options.promotion:
        return []
    return [f"opentide mutate promote --target {options.promotion_target}"]


def document_steps() -> list[str]:
    return ["opentide document"]


def header_comment(options: CiRenderOptions) -> str:
    return (
        f"# Generated by OpenTide — regenerate with "
        f"`opentide ci generate --ci {options.ci}`\n"
